// Fichier : Metier/MRP/OrdreAchatService.cs
using System;
using System.Linq;
using System.Threading.Tasks;
using Donnees;
using erp_pfc_20252026.Data.Entities;
using Microsoft.EntityFrameworkCore;
using Metier; // pour IPdfService

namespace Metier.MRP
{
    public class OrdreAchatService
    {
        private readonly ErpDbContext _db;
        private readonly IPdfService _pdfService;
        private readonly BlockchainService _blockchain;

        public OrdreAchatService(ErpDbContext db, IPdfService pdfService, BlockchainService blockchain)
        {
            _db = db;
            _pdfService = pdfService;
            _blockchain = blockchain;
        }

        public class OrdreAchatPdfModel
        {
            public string ReferenceOA { get; set; } = string.Empty;
            public string ReferencePlan { get; set; } = string.Empty;
            public DateTime DateOrdre { get; set; }

            public string CodeArticle { get; set; } = string.Empty;
            public string NomArticle { get; set; } = string.Empty;
            public string TypeProduit { get; set; } = string.Empty;

            public decimal Quantite { get; set; }
            public DateTime DateHorizonDebut { get; set; }
            public DateTime DateHorizonFin { get; set; }

            public decimal PrixUnitaire { get; set; }
            public decimal PrixTotal { get; set; }
        }

        public async Task<MRPFichier> GenererOrdreAchatAsync(
            int planificationId,
            string codeArticle,
            decimal quantite)
        {
            if (string.IsNullOrWhiteSpace(codeArticle))
                throw new ArgumentException("Code article obligatoire.", nameof(codeArticle));

            if (quantite <= 0)
                throw new ArgumentException("La quantité doit être > 0.", nameof(quantite));

            // Un seul OA par couple (plan, article)
            var oaExistant = await _db.MRPFichiers
                .FirstOrDefaultAsync(f => f.PlanificationId == planificationId
                                       && f.CodeArticle == codeArticle
                                       && f.ReferenceOF.StartsWith("OA-"));
            if (oaExistant != null)
                throw new InvalidOperationException(
                    $"Un ordre d'achat existe déjà pour l'article {codeArticle} sur ce plan (réf. {oaExistant.ReferenceOF}).");

            var plan = await _db.MRPPlans
                .Include(p => p.Lignes)
                    .ThenInclude(l => l.Produit)
                .FirstOrDefaultAsync(p => p.Id == planificationId);

            if (plan == null)
                throw new InvalidOperationException("Planification MRP introuvable.");

            var lignePlan = plan.Lignes.FirstOrDefault(l => l.Produit.Reference == codeArticle);
            if (lignePlan == null)
                throw new InvalidOperationException("Ligne MRP introuvable pour cet article.");

            var produit = lignePlan.Produit;
            if (produit == null)
                throw new InvalidOperationException("Produit introuvable pour cet article.");

            var referenceOa = $"OA-{plan.Id:D4}-{DateTime.UtcNow:yyyyMMddHHmmss}";
            var dateOrdre = DateTime.UtcNow;

            var model = new OrdreAchatPdfModel
            {
                ReferenceOA = referenceOa,
                ReferencePlan = plan.Reference,
                DateOrdre = dateOrdre,
                CodeArticle = produit.Reference,
                NomArticle = produit.Nom,
                TypeProduit = lignePlan.TypeProduit,
                Quantite = quantite,
                DateHorizonDebut = plan.DateDebutHorizon,
                DateHorizonFin = plan.DateFinHorizon,
                PrixUnitaire = produit.Cout, // On utilise le coût comme prix d'achat par défaut
                PrixTotal = produit.Cout * quantite
            };

            // Génération du PDF via la vue Razor Pages/MRP/OATemplate.cshtml
            var pdfBytes = await _pdfService.GeneratePdfFromViewAsync("MRP/OATemplate", model);

            var fichier = new MRPFichier
            {
                PlanificationId = plan.Id,
                CodeArticle = codeArticle,
                ReferenceOF = referenceOa, // On réutilise le champ ReferenceOF pour stocker la référence OA
                DateOrdre = dateOrdre,
                FichierNom = referenceOa + ".pdf",
                ContentType = "application/pdf",
                TailleOctets = pdfBytes.LongLength,
                FichierBlob = pdfBytes,
                CreeLe = dateOrdre
            };

            _db.MRPFichiers.Add(fichier);
            await _db.SaveChangesAsync();

            // Ancrage blockchain — on hash le PDF + les métadonnées clés
            var contenuAHacher = pdfBytes
                .Concat(System.Text.Encoding.UTF8.GetBytes($"|{referenceOa}|{codeArticle}"))
                .ToArray();
            await _blockchain.AncrerDocumentAsync("OA", referenceOa, contenuAHacher);

            return fichier;
        }
    }
}
