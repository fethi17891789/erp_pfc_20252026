...
namespace erp_pfc_20252026.Pages
{
    public class ProduitNewModel : PageModel
    {
        ...
        {
            Console.WriteLine("[DEBUG] ProduitNew.OnPostAsync appel�");
            ...
            // MP ou Semi-fini -> prix de vente forc� � 0
            ...
            // Co�t d'achat de base :
            // si le champ est "none" (SF / Fini / SF+Fini), on force � 0.
            ...
            // Co�ts selon le type technique
            if (NouveauProduit.TypeTechnique == TypeTechniqueProduit.MatierePremiere)
            {
                // Mati�re premi�re : on garde le co�t d'achat saisi
            ...
            {
                // Semi-fini, Fini ou les deux : seul CoutAchat est forcé à 0
                // CoutBom est géré exclusivement par BOMCreate — ne pas l'écraser ici
            ...
            // Autres charges (toujours 0 via le champ cach�)
            NouveauProduit.CoutAutresCharges = ParseDecimalFromForm(coutAutresRaw);

            // CoutTotal = CoutBom + CoutAutresCharges (champ calculé, jamais pris du formulaire)
            ...
            try
            {
                if (id > 0)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(NouveauProduit.Reference))
                    {
                        ...
                        if (refExiste)
                        {
                            ErrorMessage = "Cette r�f�rence existe d�j� pour un autre produit.";
                            return Page();
                        }
                    }
                    ...
                    // CoutBom des non-MP est géré par BOMCreate — on préserve la valeur calculée
                    if (NouveauProduit.TypeTechnique == TypeTechniqueProduit.MatierePremiere)
                    ...
                    if (ProductImage != null && ProductImage.Length > 0)
                    {
                        ...
                        Console.WriteLine($"[DEBUG] ProduitNew.OnPost - nouvelle image enregistr�e {fileName}");
                    }

                    await _context.SaveChangesAsync();
                    SuccessMessage = $"Produit {existing.Nom} mis � jour avec succ�s (ID {existing.Id}).";
                ...
                else
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(NouveauProduit.Reference))
                    {
                        ...
                        if (refExiste)
                        {
                            ErrorMessage = "Cette r�f�rence existe d�j�.";
                            return Page();
                        }
                    }
                    ...
                    if (ProductImage != null && ProductImage.Length > 0)
                    {
                        ...
                        Console.WriteLine($"[DEBUG] ProduitNew.OnPost - image enregistr�e {fileName}");
                    }

                    _context.Produits.Add(NouveauProduit);
                    await _context.SaveChangesAsync();

                    Console.WriteLine($"[DEBUG] ProduitNew.OnPost - produit cr�� ID={NouveauProduit.Id}, ReturnToBom={ReturnToBom}, BomTarget={BomTarget}, BomRowIndex={BomRowIndex}");
                    ...
                    SuccessMessage = $"Produit {NouveauProduit.Nom} cr�� avec succ�s (ID {NouveauProduit.Id}).";
                    return Page();
                }
            }
            catch (Exception ex)
        ...
        private decimal ParseDecimalFromForm(string raw)
        {
            ...
            Console.WriteLine($"[DEBUG] ParseDecimalFromForm - �chec de parse pour valeur '{raw}', normalis�e '{normalized}', valeur forc�e � 0.");
            return 0m;
        }
